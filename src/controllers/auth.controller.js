...
export const signUp = async (req, res) => {
  try {
    ...
    const otp = Math.floor(100000 + Math.random() * 900000)
      .toString()
      .padStart(6, "0"); // Generate a 6-digit OTP as a string
...
export const signInAdmin = async (req, res) => {
  try {
    ...
    const existingRole = await prisma.role.findUnique({
      where: { roleId: existingUser.roleId },
    });
...
export const signInWithGoogle = async (req, res) => {
  try {
    ...
    let filePath = null;
    if (photoUrl) {
      const { url: imageUrl, filePath: imageFilePath } =
        await uploadFileFromUrl(photoUrl);
      url = imageUrl;
